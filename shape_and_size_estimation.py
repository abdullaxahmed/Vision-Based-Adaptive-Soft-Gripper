...
vis.create_window("3D Measure", width=W, height=H)
opt = vis.get_render_option(); opt.background_color=[1,1,1]; opt.point_size=POINT_SIZE
def grid(sz,st):
    pts,lines,idx=[],[],0
    rng=np.arange(-sz,sz+1e-6,st)
    for x in rng: pts+=[[x,-sz,0],[x,sz,0]]; lines+=[[idx,idx+1]]; idx+=2
    for y in rng: pts+=[[-sz,y,0],[sz,y,0]]; lines+=[[idx,idx+1]]; idx+=2
    return o3d.geometry.LineSet(points=o3d.utility.Vector3dVector(pts),
                                lines =o3d.utility.Vector2iVector(lines))
g=grid(GRID_HALF,GRID_STEP); g.paint_uniform_color([.6,.6,.6]); vis.add_geometry(g)
pintr=o3d.camera.PinholeCameraIntrinsic(W,H,c_intr.fx,c_intr.fy,c_intr.ppx,c_intr.ppy)
vis.add_geometry(o3d.geometry.LineSet.create_camera_visualization(pintr,np.linalg.inv(np.eye(4)),0.2))
...
def detect_and_draw(frame):
    ...
    if len(approx) == 4:
        ...
        if shape == "Square":
            ...
            v      = top_edge[1] - top_edge[0]            # direction vector
...
def clamp(x,y): return max(0,min(x,W-1)),max(0,min(y,H-1))

try:
    while True:
        fs=pipe.wait_for_frames()
        d=fs.get_depth_frame(); c=fs.get_color_frame()
        if not d or not c: continue
        img=np.asanyarray(c.get_data())
        rgb_view,_,_ = detect_and_draw(img)         

        # depth-based interior points 
        gmask=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
        _,bin_=cv2.threshold(gmask,0,255,cv2.THRESH_OTSU+cv2.THRESH_BINARY)
        if cv2.countNonZero(bin_)>bin_.size//2: bin_=cv2.bitwise_not(bin_)
        bin_=cv2.erode(bin_,np.ones((ERODE_PX,ERODE_PX),np.uint8))
        cnt= max(cv2.findContours(bin_,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)[0],
                 key=cv2.contourArea) if np.count_nonzero(bin_) else None
        if cnt is None: cv2.imshow("RGB View",rgb_view); continue
        x,y,w,h=cv2.boundingRect(cnt); cx=x+w//2; cy=y+h//2
        d_np = np.asanyarray(d.get_data(),dtype=np.float32)*depth_scale
        patch=d_np[y:y+h,x:x+w]; vals=patch[(patch>0)&(patch<MAX_DEPTH_M)]
        ...
        p0=(x+int(xs[0]), y+row); p1=(x+int(xs[-1]), y+row)
        

        z0=d.get_distance(*p0); z1=d.get_distance(*p1)
        ...
        dist_cm=np.linalg.norm(P1-P0)*100
        cv2.putText(rgb_view,f"{dist_cm:.1f}cm",(cx-40,cy+30),
                    cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,255,255),2)

       
        pc=rs.pointcloud(); pc.map_to(c)
        v=np.asanyarray(pc.calculate(d).get_vertices()).view(np.float32).reshape(-1,3)
        col=np.asanyarray(c.get_data()).reshape(-1,3)/255.0
    ...
    pipe.stop(); cv2.destroyAllWindows(); vis.destroy_window()
